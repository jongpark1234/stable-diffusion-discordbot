...
class txt2img(commands.Cog):
    ...
    async def txt2img(
        ...
        negative_prompt: str
            AI에게 하지 말아야 할 것을 알려주는 문자열 입니다. 기본값 - EasyNegative, extra fingers, fewer fingers, NSFW
        width: int
            이미지의 너비 입니다. (width는 64에서 1024 사이의 정수여야 합니다.) 기본값 - 512
        height: int
            이미지의 높이 입니다. (height는 64에서 1024 사이의 정수여야 합니다.) 기본값 - 768
        steps: int
            AI가 반복해서 그림을 그릴 횟수입니다. ( 기본값: 40 )
        sampler: str
            이미지의 샘플러입니다. ( 기본값: DPM++ SDE Karras )
        seed : bool
        ...
        payload = {
            ...
            'sampler_index': sampler if type(sampler) == str else sampler.value,
            'sampler_name': sampler if type(sampler) == str else sampler.value,
            'steps': steps,
            'width': width,
            'height': height,
            'cfg_scale': 7,
